...
function mouseMove(~, ~)
    ...
    if (settings.showIDs == true)
        ...
        if ((channel1Distance < channel2Distance && settings.currentChannel == 3) || settings.currentChannel == 1)
            ...
            set(settings.idLabelHandle, 'String', ['ID = ' num2str(settings.seedPoints1Filtered(channel1Index, 1)) ', Pos = ' num2str(settings.seedPoints1Filtered(channel1Index, 3:5)) ', Radius = ' num2str(settings.seedPoints1Filtered(channel1Index, 2)*settings.physicalSpacingXY)]);
        ...
        if ((channel1Distance >= channel2Distance && settings.currentChannel == 3) || settings.currentChannel == 2)
            ...
            set(settings.idLabelHandle, 'String', ['ID = ' num2str(settings.seedPoints2Filtered(channel2Index, 1)) ', Pos = ' num2str(settings.seedPoints2Filtered(channel2Index, 3:5)) ', Radius = ' num2str(settings.seedPoints2Filtered(channel2Index, 2)*settings.physicalSpacingXY)]);
